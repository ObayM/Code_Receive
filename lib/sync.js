...
async function getImapClient() {
    ...
    // Reconnect on close
    client.on("close", () => {
        logger.info("[SYNC] IMAP connection closed");
        imapClient = null;
    });

    client.on("error", (err) => {
        logger.error({ err }, "[SYNC] IMAP error");
        imapClient = null;
    });
    ...
    imapClient = client;
    logger.info(`[SYNC] ✅ Connected to ${config.host} as ${config.user}`);
    return client;
}
...
async function syncOnce() {
    ...
    try {
        ...
        logger.info({ searchSince }, `[SYNC] searching for messages since ${searchSince.toISOString()}`);
        ...
        if (!messages.length) {
            logger.info("[SYNC] No new messages found in search window.");
            return;
        }

        logger.info(`[SYNC] 📧 Found ${messages.length} emails to process`);
        ...
        for await (const message of client.fetch(messages, {
            ...
            // Log every processed email details
            logger.info({
                msgId: message.uid,
                from,
                to,
                subject,
                date: receivedAt
            }, `[SYNC] Processing email: "${subject}" from ${from}`);

            const text = (parsed.text || "") + " " + (parsed.html || "");
            const codes = extractCodes(text);

            if (codes.length > 0) {
                logger.info({ codes }, `[SYNC] Found ${codes.length} potential codes in "${subject}"`);
                ...
                logger.info(`[SYNC] No codes found in email: "${subject}"`);
            }
        }

        if (newCodes.length === 0) {
            logger.info("[SYNC] No valid codes extracted from any messages.");
            return;
        }
        ...
        const codesToInsert = newCodes.filter((c) => {
            ...
            if (exists) {
                logger.info({ code: c.code, email: c.email }, `[SYNC] Duplicate code found, skipping.`);
            }
            return !exists;
        });

        if (codesToInsert.length === 0) {
            logger.info("[SYNC] ✅ No new unique codes to save (all duplicates)");
            return;
        }
        ...
        logger.info({ count: codesToInsert.length }, `[SYNC] ✅ Saved ${codesToInsert.length} new code(s) to database`);
...
export function startSyncLoop() {
    ...
    logger.info("[SYNC] 🔄 Starting background sync loop");
