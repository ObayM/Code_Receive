import { getGmailClient, resetGmailClient, isAuthError, setupGmailWatch } from "./gmail.js";
...
/**
 * Perform a single sync operation, optionally starting from a historyId
 */
export async function syncOnce(historyId = null) {
    ...
    try {
        ...
        logger.debug(`[SYNC] Searching Gmail: q="${query}"`);
        ...
        if (historyId) {
            try {
                logger.debug(`[SYNC] Using history list starting from ${historyId}`);
                let hPageToken;
                do {
                    const hRes = await gmail.users.history.list({
                        userId: "me",
                        startHistoryId: historyId,
                        pageToken: hPageToken
                    });
                    if (hRes.data.history) {
                        for (const h of hRes.data.history) {
                            if (h.messagesAdded) {
                                allMessageIds.push(...h.messagesAdded.map(ma => ma.message.id));
                            }
                        }
                    }
                    hPageToken = hRes.data.nextPageToken;
                } while (hPageToken);
            } catch (err) {
                logger.warn(`[SYNC] history.list failed for ${historyId}: ${err.message}. Falling back to search.`);
                historyId = null; // Fallback
            }
        }

        if (!historyId) {
            // Paginate through results using search query
        do {
        ...
        if (!allMessageIds.length) {
            logger.debug("[SYNC] No messages found in lookback window.");
            return;
        }

        logger.debug(`[SYNC] 📧 Found ${allMessageIds.length} message(s) in window`);
        ...
        for (const msgId of allMessageIds) {
            try {
                ...
                if (codes.length > 0) {
                    logger.debug({ codes }, `[SYNC] Found ${codes.length} code(s) in "${subject}"`);
        ...
        if (newCodes.length === 0) {
            logger.debug("[SYNC] No valid codes extracted.");
            return;
        }
        ...
        if (codesToInsert.length === 0) {
            logger.debug("[SYNC] ✅ No new unique codes (all duplicates)");
            return;
        }

        await prisma.code.createMany({ data: codesToInsert });
        logger.debug({ count: codesToInsert.length }, `[SYNC] ✅ Saved ${codesToInsert.length} new code(s)`);
...
/**
 * Ensures the Gmail watch is active, renewing if necessary.
 */
export async function ensureWatch() {
    try {
        ...
        // Renew watch if it expires within 1 day (or doesn't exist)
        if (!state || !state.expiration || (state.expiration.getTime() - now.getTime() < 24 * 60 * 60 * 1000)) {
            logger.debug("[SYNC] Watch not found or expiring soon, setting up Gmail watch...");
            const watchData = await setupGmailWatch();
            if (watchData && watchData.expiration) {
                const expDate = new Date(Number(watchData.expiration));
                logger.debug({ expiration: expDate }, "[SYNC] Gmail watch setup successful");
                
                await prisma.syncState.upsert({
                    where: { id: state ? state.id : 1 },
                    update: { expiration: expDate, historyId: watchData.historyId },
                    create: { id: 1, expiration: expDate, historyId: watchData.historyId }
                });
            }
        }
    } catch (err) {
        logger.error({ err }, "[SYNC] Failed to ensure watch");
    }
}
...
export function startSyncLoop() {
    ...
    logger.debug("[SYNC] 🔄 Starting background sync loop (Gmail API)");

    // Ensure watch is active and run first sync
    ensureWatch()
        .then(() => syncOnce())
        .catch((err) => logger.error({ err }, "[SYNC] Initial sync error"));

    // We can increase the interval significantly since webhooks handle realtime
    setInterval(() => {
        ensureWatch()
            .then(() => syncOnce())
            .catch((err) => logger.error({ err }, "[SYNC] Interval sync error"));
    }, 60000); // 1 minute fallback instead of 10 seconds
}
